...
* **entity-interface**, 系统种子数据类型接口，主要用于 TypeScript 编译器的类型识别。客户端的代码导出功能导出的文件，直接复制过来的。客户端也会复制一份同样的代码。
...
TypeORM 只有一个入口，能够传入实体定义，就是 `createConnection`。需要在这个函数调用前，解析完元数据，分离出实体定义。这个模块的 `TypeOrmService` 完成这些 connection 的管理工作，依赖的 schema 模块的 `SchemaService`,提供如下接口：
